...
#: 因为这是电调本身的属性，不随电机编号走。
#:
#: 为什么需要：电调的真实中位往往不在 1500µs。不校的话"油门 0"就是一点点正油门，
#: 桨会持续爬行 —— 耗电、持续扰动姿态，水下还会一直漂。
#:
#: 测法（2026-07-29 用过）：网页独立电机模式逐档降低脉宽，找到停转的上/下边缘，
#: 取死区正中间（不要取边缘，边缘没余量，温漂或电压变化就会复发爬行）。
#:   通道 6/7（电机 7/8）实测：上边缘 ≈1492.5µs、下边缘 ≈1457.5µs，
#:                            死区宽 35µs、中心 1475µs → 偏置 −25µs。
#:                            余量两侧各 17.5µs；反向权限从 500µs 降到 475µs（−5%）。
#:   通道 0..5（电机 1-6）**尚未实测**，暂为 0。这批很可能是同型号电调、有同样的偏移，
#:                            建议用同样方法量一遍，否则水下会有持续的深度漂移与姿态扰动。
MOTOR_TRIM_US = (0, 0, 0, 0, 0, 0, -25, -25)
